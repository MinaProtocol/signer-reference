...
let test_bit = (t, i) =>
  equal(
    log_and(Big_int.unit_big_int, shift_right(t, i)),
...
let of_bytes = x =>
  String.foldi(x, ~init=Big_int.zero_big_int, ~f=(i, acc, c) =>
...
let of_yojson =
  ...
  | _ => Error("Nat.of_yojson: Expected string");

/** this serialization is not used for fields */
include Binable.Of_stringable({
  type nonrec t = t;
